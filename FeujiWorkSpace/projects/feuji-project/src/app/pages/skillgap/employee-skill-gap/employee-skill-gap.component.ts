  ...
  onSelectskillCat(referenceDetailId: any) {
    ...
    this.empskillService.getSkillsOfEmployee(this.empMail, this.referenceDetailId).subscribe(
      (response) => {
        ...
        console.log(this.getMySkills)
        this.barChartLabels = this.getMySkills.map(skill => skill.skillName);
        this.barChartData[0].data = this.getMySkills.map(skill => this.mapLevelToNumeric(skill.exReferenceDetailsValues));
        this.barChartData[1].data = this.getMySkills.map(skill => this.mapLevelToNumeric(skill.acReferenceDetailsValues));
      },
      (error) => {
        Swal.fire({
